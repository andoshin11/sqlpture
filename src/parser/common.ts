...
export type ParseTableSpecifier<
  T
> = T extends `${infer Source} AS ${infer Alias}`
...
export type ParseIdentifier<T> = T extends ""
  ? never
...
export type ParseStringLiteral<T> = T extends `"${infer Value}"${infer Rest}`
  ? [StringLiteral<Value>, Rest]
  : T extends `'${infer Value}'${infer Rest}`
  ? [StringLiteral<Value>, Rest]
  : ParseMemberExpression<T>;

export type ParseCallExpression<T> = Trim<T> extends ""
  ? never
  : ParseStringLiteral<Trim<T>> | ParseParenthesizedExpression<T>;

export type ParseBinaryExpression<T> = ParseCallExpression<T> extends [
  infer Left,
  infer R1
]
...
export type ParseLogicalExpression<T> = ParseBinaryExpression<T> extends [
  infer Left,
  infer R1
]
  ? Tokenize<R1> extends [infer Op, infer R2]
...
export type ParseParenthesizedExpression<
  T
> = T extends `(${infer Content})${infer Rest}`
  ? [ParseExpression<Content>, Rest]
  : never;

export type ParseFieldSpecifierList<T> = T extends `${infer Head},${infer Tail}`
  ...
  : T extends `${infer Head} ${infer Alias}`
  ? [
      FieldSpecifier<
        Trim<ParseMemberExpression<Head>[0]>,
        Trim<ParseIdentifier<Alias>[0]>
