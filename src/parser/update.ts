...
> = _T extends `UPDATE ${infer TableName} SET ${infer Values}`
  ? Values extends `${infer R0}RETURNING${infer R1}`
    ? { tableName: TableName } & ParseReturningClause<_T> &
        ParseSetClause<TrimRight<R0>>
    : { tableName: TableName } & {
        returningFields: [];
      } & ParseSetClause<Values>
  : never;

type ParseSetClause<T> = Trim<T> extends `${infer R1}WHERE ${infer WHERE}`
  ? {
      values: ParseAssignmentExpressionList<TrimRight<R1>>;
    } & ParseWhereClause<WHERE>
  : { values: ParseAssignmentExpressionList<T> } & {
...
type ParseAssignmentExpressionList<T> = T extends `${infer Head},${infer Tail}`
  ...
  ? [
      AssignmentExpression<Identifier<Trim<Head>>, ParseValue<Trim<Value>>>,
      Tail
    ]
...
type ParseAssignmentExpression<T> = T extends `${infer Key} = ${infer Value}`
  ? AssignmentExpression<Identifier<Key>, ParseValue<Trim<Value>>>
  : never;
