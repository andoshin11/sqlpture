...
> = Node extends InsertStatement<infer TableName, infer Fields, infer Values, infer ReturningFields>
  ? true extends (ValidateFields<DB, Node> & ValidateValues<DB, Node> & ValidateReturningFields<DB, Node>)
    ? true
    : false
  : false
...
> = Node extends InsertStatement<infer TableName, infer Fields, infer Values, infer ReturningFields>
  ? TableName extends keyof DB['schema']
    ? { [K in keyof Fields]: Fields[K] extends keyof DB['schema'][TableName] ? true : false } extends true[]
      ? true
      : false
    : false
  : false
...
> = Node extends InsertStatement<infer TableName, infer Fields, infer Values, infer ReturningFields>
  ? TableName extends keyof DB['schema']
    ? { [K in keyof Fields]: Fields[K] extends keyof DB['schema'][TableName] ? DB['schema'][TableName][Fields[K]] : never }
    : never
  : never
