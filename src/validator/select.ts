...
> = Node extends SelectStatement<
  ...
  infer Offset
>
  ? true extends ValidateFieldList<DB, Node> &
      ValidateFromClause<DB, Node> &
      ValidateJoinClauses<DB, Node> &
      ValidateWhereClause<DB, Node>
    ? true
    : false
  : false;

// WIP
export type ValidSelectStatement<DB extends Database> = ValidFromClause<DB> &
  ValidJoinClauses<DB> &
  ValidWhereClause<DB> &
  ValidLimitClause<DB>;
...
export type ValidateFromClause<
  ...
  Node extends SelectStatement
> = Node extends {
  from: TableSpecifier<Identifier<infer Source>, Identifier<any>>;
}
...
export type ValidFromClause<DB extends Database> = {
  from: TableSpecifier<Identifier<string & keyof DB["schema"]>, any>;
};

type ToJoinSourceList<
  Joins extends JoinSpecifier[],
  List extends string[] = []
> = Joins extends [infer Head, ...infer Tail]
...
type _CheckExpressionValidity<
  ...
  _JoinedSchema = _GetJoinedSchema<DB, Node>
> = _JoinedSchema extends JoinedSchema<DB>
  ? EXP extends VariableExpression
    ? true
    ...
    ? [
        _CheckExpressionValidity<DB, Node, Left>,
        _CheckExpressionValidity<DB, Node, Right>
      ] extends [true, true]
      ? true
      : false
...
>
  ? ToJoinSourceList<Joins> extends Array<keyof DB["schema"]>
    ? _CheckJoinConnecter<DB, Node> extends true[]
      ? true
      : false
    : false
  : false;

export type ValidJoinClauses<DB extends Database> = {
  joins: Array<
    InnerJoinSpecifier<
      TableSpecifier<Identifier<string & keyof DB["schema"]>, any>,
      any
    >
  >;
};
...
export type ValidLimitClause<DB extends Database> = {
